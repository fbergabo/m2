...
// Computes the boltzmann factors
...
int ising_init (struct ising *model, int lx, int ly, double jc, double h,
    double t)
{
    ...
    model->s = s;
    //allocates spins to each element
    for (int i = 0; i < lx; i++)
    {
        for (int j = 0; j < ly; j++)
        {
...
//function used to calculate magnetization per spin
...
//function for calculating energy per spin
